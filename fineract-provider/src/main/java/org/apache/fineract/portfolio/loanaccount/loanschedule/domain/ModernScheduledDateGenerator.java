 ...
 * {@code bba-loan-schedule-generator}) as the new implementation behind the {@link ScheduledDateGenerator}
 * abstraction. Callers are flipped onto it one at a time through {@link ScheduledDateGeneratorFactory}; once every
 * caller uses it, the legacy {@code DefaultScheduledDateGenerator} is retired.
 *
 * <p>
 * Its observable behavior is identical to the legacy generator — the repayment-schedule date arithmetic is the same.
 * Behavioral identity is pinned by the characterization golden-masters
 * ({@code ModernScheduledDateGeneratorCharacterizationTest} at the unit level and
 * {@code LoanScheduleCalculationCharacterizationTest} end-to-end through the calculate-schedule endpoint). The value of
 * the swap is the decoupling and the proven, reversible replacement mechanism, not a change in date math.
 */
public class ModernScheduledDateGenerator implements ScheduledDateGenerator {
    ...
    private AdjustedDateDetailsDTO recursivelyCheckNonWorkingDaysAndHolidaysAndWorkingDaysExemptionToGenerateNextRepaymentPeriodDate(
        ...
        /**
         * Check Changed Schedule Date is holiday or is not a working day Then re-call this method to get the non holiday
         * and working day
         */
